...
OpenWrap.python.prototype.startServer = function(aPort, aSendPort, aFn) {
	...
	if (isUnDef(this.token)) {
		...
		s += "			try:\n";
		s += "				exec(mm['e'])\n";
		s += "				mm['out'] = myStdOut.getvalue()\n";
		s += "				mm['err'] = myStdErr.getvalue()\n";
		s += "			except:\n";
		s += "				mm['err'] = str(sys.exc_info())\n";
		s += "\n";
		s += "		self.request.sendall(json.dumps(mm))\n";
		s += "\n";
		s += "server = socketserver.TCPServer(('127.0.0.1', " + aSendPort + "), oafHandler)\n";
		s += "server.serve_forever()\n";

		plugin("Threads");
		var threads = new Threads();
		threads.addSingleThread(function() { print("Starting server on " + aSendPort); sprint(af.sh("python -", s, void 0, void 0, void 0, true)); });
	}
...
OpenWrap.python.prototype.execPM = function(aPythonCode, aInput, throwExceptions) {
	...
	var res = af.sh(this.python + " -", code, void 0, void 0, void 0, true);
	var rres = [];
...
OpenWrap.python.prototype.iexec = function(aPythonCode, aInput, aOutputArray, throwExceptions) {
	if (this.version < 0) throw "Appropriate Python version not found. Please setPython to a python version 2 or version 3 command-line interpreter.";
	...
	var res = jsonParse(ow.ojb.socket.string2string("127.0.0.1", this.sport, stringify({ e: code }, void 0, "")+"\n"));
	var rres = [];
	if (res.out.indexOf(delim) >= 0) {
		rres = res.out.split(new RegExp("^" + delim + "\r?\n", "mg"));
	}
	if (isDef(rres[0]) && rres[0] != "") print(rres[0]);
	if (isDef(res.err) && res.err != "") {
		if (throwExceptions) {
			printErr(res.err);
			throw "python: " + String(res.err);
		} else {
			printErr(res.err);
		}
	}
