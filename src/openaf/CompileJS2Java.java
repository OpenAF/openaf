...
public class CompileJS2Java {
		...
		try {
			...
			if (args.length > 3 && args[3] != null && !args[3].isEmpty()) {
				try {
					...
					System.err.println("Using RHINO_LEGACY method.");
				}
			}
    ...
    /**
     * Compile JavaScript to Java classes using the default compilation method
    ...
    public static void compileToClasses(String classfile, String script, String path, CompilationMethod method) {
		...
		// Avoid embedding debug info / the full source in the generated class to
		// prevent ConstantPool "Too big string" for large scripts.
		ce.setGenerateDebugInfo(false);
		ce.setGeneratingSource(false);

		// Optimization level is the same across methods; interpreted mode is
		// incompatible with class file generation.
    ...
    private static Object[] compileToClassFilesDefault(ClassCompiler cc, String script, String sourceName, int lineno, String className) {
        return compileToClassFilesInternal(cc, script, sourceName, lineno, className);
    }
    ...
    /**
     * Compiles the script via Rhino's Codegen directly, then clears the JSDescriptor's
     * raw source before serializing the descriptor bytecode - working around Rhino's
     * ClassCompiler unconditionally embedding the full source as a class constant,
     * which overflows the JVM's 65535-byte constant pool UTF8 limit for large scripts.
     */
    private static Object[] compileToClassFilesInternal(ClassCompiler cc, String script, String sourceName, int lineno, String className) {
    ...
    /**
     * Scopes each descriptor's rawSource down to just that function's own slice
     * (using its rawSourceStart/rawSourceEnd offsets into the whole-script text),
     * instead of blanking it outright. Rhino's Function.prototype.toString() for
     * compiled functions is reconstructed straight from this rawSource, so wiping
     * it unconditionally (as a prior version of this method did) silently breaks
     * toString() for every function in the script - including small ones - which
     * in turn breaks any code relying on it (e.g. code that does
     * eval(fn.toString()) to reconstruct a function after serialization).
     *
     * The top-level script descriptor has no valid start/end slice (Rhino marks
     * it with -1/-1, meaning "whole source") and is exactly the multi-KB text that
     * risks overflowing the JVM's 65535-byte constant pool UTF8 limit, so it is
     * still blanked - nothing calls .toString() on the script object itself.
     * Individual function descriptors get their own (typically tiny) slice
     * preserved, and only fall back to blanking if that slice is itself still too
     * large to safely fit as a class constant.
     */
    private static void clearDescriptorRawSources(JSDescriptor.Builder<?> builder) {
        if (builder == null) {
            return;
        }
        String full = builder.rawSource;
        int start = builder.rawSourceStart;
        int end = builder.rawSourceEnd;
        if (full != null && start >= 0 && end >= start && end <= full.length()) {
            String slice = full.substring(start, end);
            if (utf8Length(slice) > MAX_STRING_LITERAL) {
                builder.rawSource = "";
                builder.rawSourceStart = 0;
                builder.rawSourceEnd = 0;
            } else {
                builder.rawSource = slice;
                builder.rawSourceStart = 0;
                builder.rawSourceEnd = slice.length();
            }
